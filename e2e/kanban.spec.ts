...
test('Einfügen (P1.1): Board = 3 Spalten, die Musterkarte liegt markiert in der ersten', async ({ page }) => {
  ...
  // frisch eingefügt ist das Board selektiert: "+ Spalte" ja. "+ Karte"
  // gibt es seit 2026-07-10 NICHT mehr (eine Zeile = eine Karte).
  ...
  // Spalte anklicken -> weiterhin KEIN "+ Karte"; der Board-Anstecker
  // "+ Spalte" bleibt (die Auswahl liegt weiter im Board).
...
test('"+ Spalte" erweitert leer; KEIN "+ Karte"; Musterkarte ohne Kreuzchen (2026-07-10: eine Zeile = eine Karte)', async ({ page }) => {
  await freshEditor(page)
  await insertBoard(page)
  ...
  // Die Musterkarte ist das Gestaltungsobjekt: Auswahl zeigt die
  // Muster-Markierung, aber KEIN Entfernen-Kreuzchen (unlöschbar) und
  // nirgends einen "+ Karte"-Anstecker — die Kartenanzahl bestimmen
  // allein die SoftEngine-Daten.
...
test('Kreuzchen: leere Spalte ohne Rückfrage; Spalte mit Musterkarte ist GESCHÜTZT (bleibt stehen)', async ({ page }) => {
  await freshEditor(page)
  await insertBoard(page)
  ...
  // Spalte mit der Musterkarte: Löschschutz (2026-07-10) — der Klick
  // erklärt per alert() und löscht NICHTS; der Store verweigert zusätzlich.

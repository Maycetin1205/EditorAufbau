// Editor
// Store-Kern (U4): hält nur einen serialisierbaren BlockNode-Baum (flache
// Map + Wurzel) + Auswahl und benachrichtigt React per Subject. Position =
// Verschachtelung + Reihenfolge (Flow), keine Koordinaten.
// Alles Weitere ist delegiert: treeOps (Baumoperationen), history (Undo/
// Redo/Gesten-Klammer), persistence (Speichern/Laden inkl. U1-Notfallkopie),
// migrations (alte Stände), templateRules (Musterkarte/Löschschutz).
...
import { History, type EditorSnapshot } from './history'
import { loadFromStorage, saveToStorage, SAVE_DEBOUNCE_MS } from './persistence'
...
import { isRemoveProtected, templateMarkFor } from './templateRules'
import { deepClone } from '../lib/deepClone'

// Der Persistenz-Wächter (persistence.test) greift auf die Notfallkopie
// über diesen Store zu — der Schlüssel bleibt hier erreichbar.
...
export class Editor extends Subject<Editor> {
  ...
  private _version = 0
  private _history = new History(HISTORY_LIMIT)
  ...
  // Vor jeder Änderung: Stand in die Historie (innerhalb einer Geste
  // unterdrückt die History weitere Snapshots selbst).
  private pushHistory(): void {
    this._history.record(this.snapshot())
  }

  beginTransaction(): void {
    this._history.beginTransaction(() => this.snapshot())
  }

  endTransaction(): void {
    this._history.endTransaction()
  }

  private restore(snapshot: EditorSnapshot | undefined): void {
    if (!snapshot) return
    this._tree = snapshot.tree
    this._selectedId = snapshot.selectedId
    this.notify(this)
  }

  undo(): void {
    this.restore(this._history.undo(() => this.snapshot()))
  }

  redo(): void {
    this.restore(this._history.redo(() => this.snapshot()))
  }
  ...
  isInSubtree(ancestorId: string, id: string): boolean {
    return isInSubtree(this._tree, ancestorId, id)
  }
  ...
  // Musterkarten-Markierung — Definition in templateRules (Export und
  // Laufzeit nutzen dieselbe treeQuery-Grundlage).
  templateMarkFor(id: string): string | undefined {
    return templateMarkFor(this._tree, id)
  }

  // Löschschutz — Definition in templateRules.
  isRemoveProtected(id: string): boolean {
    return isRemoveProtected(this._tree, id)
  }
  ...
  private scheduleSave(): void {
    ...
    this._saveTimer = setTimeout(() => {
      saveToStorage(this._tree, this._selectedId)
    }, SAVE_DEBOUNCE_MS)
  }
}

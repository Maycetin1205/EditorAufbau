...
`;function Ua(e){return{rohzeilen:e.map(e=>e.rohzeile),datenzeilen:e.map(e=>[...e.zellen]),zusatzzeilen:e.map(e=>(e.zusatz??[]).map(e=>({...e})))}}function Wa(e,t,n){return t===``||n===``?[...e]:e.filter(e=>Ir(k(e,t))===n)}function Ga(e){let t=e.getAttribute(`source`)??``;if(t===``)return null;let n=O(I().FF_DATA_SOURCES,t);return n?{quelle:n,zeilen:Wa(Nt(I().SEDATA,n.name,n.tableId),e.getAttribute(`tagfield`)??``,Vr()),lies:di(e)}:null}function Ka(e){return ta(e.getAttribute(`spalten`)??``)}function qa(e,t,n){let r={};for(let i of V(e.art).zusatzFelder??[]){let a=e.felder?.[i.key]??``;a!==``&&(r[i.key]=n(t,a))}return r}function Ja(e,t){let n=O(I().FF_DATA_SOURCES,e.getAttribute(`source`)??``);return n?jt(n,t):``}function Ya(e){let t=Ga(e);if(!t){e.datenzeilen=[],e.zusatzzeilen=[];return}let n=Ka(e),{rows:r,gefiltert:i}=tn(e,t.zeilen),a=Jt(N(e),r,e=>e)[0]??-1,o=t.lies;e.datenGeliefert=!0,e.rohzeilen=r,e.auswahlIndex=a,e.durchAuswahlGefiltert=i,e.datenzeilen=r.map(e=>n.map(t=>t.feld===``?``:o(e,t.feld))),e.zusatzzeilen=r.map(e=>n.map(t=>qa(t,e,o)))}var Xa=oi({hydriere:Ya}),Za=Xa.connect,Qa=Xa.disconnect;function $a(e,t){let n=[];return e.forEach((e,r)=>{Kr(e,t)&&n.push(r)}),n}function eo(e,t){return!e&&t.trim()!==``}function to(e,t,n){return e&&t&&n===0}function no(e){if(!e.hatQuelle)return`— Datensätze`;let t=e.auswahlAktiv?` · durch Auswahl gefiltert`:``,n=e=>e===1?`Datensatz`:`Datensätze`,r=e=>e===1?`Datensatz`:`Datensätzen`;return e.suchtAktiv?e.sichtbar===0?`Kein Treffer von ${e.gesamt} ${r(e.gesamt)}`+t:`${e.sichtbar} von ${e.gesamt} ${r(e.gesamt)}`+t:(e.gesamt===0?`Keine Datensätze`:`${e.gesamt} ${n(e.gesamt)}`)+t}function ro(e,t,n){return g`<div class="steuerung">
    <button

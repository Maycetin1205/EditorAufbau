...
`,ze=class extends R{constructor(...e){super(...e),this.variant=`info`,this.text=`Hinweis`}static{this.blockType=`badge`}static{this.tagName=`ff-badge`}static{this.displayName=`Status-Chip`}static{this.category=`anzeige`}static{this.resizableWidth=!1}static{this.defaultProps={variant:`info`,text:`Hinweis`}}static{this.customProperties=[B(`variant`,`Bedeutung des Chips — bestimmt die Farbe.`)]}static{this.styles=[R.styles,Re]}render(){return E`<span
    ...
    `]}onSlotChange(e){let t=e.target;this._count=t.assignedElements().filter(e=>!e.hasAttribute(`data-ff-editor-helper`)).length}render(){return E`<div class="col v-${z(this.variant)}">
    ...
    </div>`}};L([I()],W.prototype,`variant`,void 0),L([I()],W.prototype,`heading`,void 0),L([Me()],W.prototype,`_count`,void 0),R.defineAndRegister(W);var Ve=[{id:`standard-put`,name:`Standard-Schreiben (PUT)`,verb:`PUT_RELATION`,nr:`174`,params:[`{FELD_POS}`,`{FELD_LEN}`,`L`,`{PINDEX}`,`{RELID}`,`{VALUE}`]}];function He(e){return Ve.find(t=>t.id===e)}function Ue(e){return e.replace(/^IDB/,``)}function We(e){let t=/^(\d+)_(\d+)$/.exec(e);return t?{pos:t[1],len:t[2]}:null}function Ge(e,t){return e.params.map(e=>e.replace(/\{([A-Z_]+)\}/g,(e,n)=>String(t[n]??``)))}function G(e){return typeof e==`object`&&!!e}function Ke(e,t){if(!(!Array.isArray(e)||t===``)){for(let n of e)if(!(!G(n)||n.id!==t)&&!(typeof n.name!=`string`||typeof n.tableId!=`string`))return{id:t,name:n.name,tableId:n.tableId,indexField:typeof n.indexField==`string`?n.indexField:``}}}function qe(e){return e==null?``:String(e).trim()}function K(e,t){if(!G(e)||t===``)return``;let n=qe(e[t]);if(n!==``)return n;let r=/^(\d+)_(\d+)$/.exec(t);if(!r)return``;let i=qe(e.SATZNEU??e.SATZ??e.satzneu??e.satz??e.RAW??e.raw);if(i===``)return``;let a=Number(r[1]),o=Number(r[2]);return o<=0?``:i.substring(a,a+o).trim()}function Je(e,t,n){if(!G(e)||t===``)return!1;let r=!1;Object.prototype.hasOwnProperty.call(e,t)&&(e[t]=n,r=!0);let i=/^(\d+)_(\d+)$/.exec(t);if(i){let t=[`SATZNEU`,`SATZ`,`satzneu`,`satz`,`RAW`,`raw`].find(t=>typeof e[t]==`string`);if(t){let a=e[t],o=Number(i[1]),s=Number(i[2]);if(s>0){let i=n.length>s?n.slice(0,s):n.padEnd(s,` `),c=a.length<o?a.padEnd(o,` `):a;e[t]=c.slice(0,o)+i+c.slice(o+s),r=!0}}}return r}function q(e){if(!G(e))return Array.isArray(e)?e:[];let t=[e.Zeilen,e.zeilen,e.Saetze,e.saetze,e.Rows,e.rows,e.Daten,e.daten];for(let e of t){if(Array.isArray(e))return e;if(typeof e==`string`)try{let t=JSON.parse(e);if(Array.isArray(t))return t}catch{}}return[]}function J(e,t){return qe(e).toLowerCase()===t.trim().toLowerCase()}function Ye(e,t,n){if(!G(e)||!G(e.Daten))return[];let r=e.Daten,i=r.SEFileLoop;if(Array.isArray(i)){for(let e of i)if(G(e)&&(J(e.ALIAS,t)||J(e.alias,t))){let t=q(e);if(t.length>0)return t}}else if(G(i))for(let e of Object.keys(i)){let n=i[e];if(J(e,t)||G(n)&&(J(n.ALIAS,t)||J(n.alias,t))){let e=q(n);if(e.length>0)return e}}let a=r.Tabellen;if(G(a)){let e=[t,t.toUpperCase(),t.toLowerCase(),n];for(let t of e)if(t!==``&&t in a){let e=q(a[t]);if(e.length>0)return e}for(let e of Object.keys(a))if(J(e,t)){let t=q(a[e]);if(t.length>0)return t}}return[]}function Xe(e,t){let n=e.trim().toLowerCase();if(n!==``)for(let e=0;e<t.length;e++){let r=t[e].trim().toLowerCase();if(r!==``&&r===n)return e}return 0}function Y(){return globalThis}function Ze(){let e=Y();return G(e.SEDATA)&&G(e.SEDATA.Daten)}function Qe(){let e=Y();try{e.selib?.Json?.InitializeERPConnection?.()}catch{}try{typeof e.InitialisiereSchnittstelle==`function`&&e.InitialisiereSchnittstelle()}catch{}}function $e(){let e=Y();try{typeof e.ResetDataBasis==`function`&&e.ResetDataBasis()}catch{}try{typeof e.InitialisiereDatenBasis==`function`&&e.InitialisiereDatenBasis()}catch{}}var et=new Set,tt=new WeakMap,nt=!1,rt=W.tagName,it=V.tagName;function at(e){return Array.from(e.children).filter(e=>e.tagName.toLowerCase()===rt)}function ot(e){return Array.from(e.children).filter(e=>e.tagName.toLowerCase()===it)}function st(e){return Fe().find(t=>t.tagName===e.toLowerCase())?.bindableSpots??[]}function ct(e){let t=e.getAttribute(`source`)??``,n=e.getAttribute(`statusfield`)??``;if(t===``||n===``)return;let r=Ke(Y().FF_DATA_SOURCES,t);if(!r)return;let i=at(e);if(i.length===0)return;let a=tt.get(e);if(!a)for(let t of i){let n=ot(t)[0];if(n){a=n.cloneNode(!0),tt.set(e,a);break}}if(!a)return;let o=Ye(Y().SEDATA,r.name,r.tableId),s=i.map(e=>e.getAttribute(`statusvalue`)??``),c=st(a.tagName);for(let e of i)ot(e).forEach(e=>e.remove());for(let e of o){let t=a.cloneNode(!0);i[Xe(K(e,n),s)].appendChild(t);for(let n of c){let r=t.getAttribute(`${n.prop.toLowerCase()}field`)??``;r!==``&&(t[n.prop]=K(e,r))}let o=K(e,r.indexField);o!==``&&(X.set(t,{row:e,pindex:o}),t.draggable=!0)}}var X=new WeakMap,Z=null,lt=new WeakSet;function ut(e,t){for(let n of t.composedPath())if(n instanceof HTMLElement&&n.tagName.toLowerCase()===rt&&e.contains(n))return n;return null}function dt(e,t,n,r){let i=Y();if(typeof i.basisHTML_SND_MSG!=`function`)return;let a=He(`standard-put`),o=Ke(i.FF_DATA_SOURCES,e.getAttribute(`source`)??``),s=We(t);!a||!o||!s||i.basisHTML_SND_MSG(a.verb,{NR:a.nr,PARAMS:Ge(a,{FELD_POS:s.pos,FELD_LEN:s.len,PINDEX:n,RELID:Ue(o.tableId),VALUE:r})})}function ft(e,t){if(!Z||Z.board!==e)return;let n=X.get(Z.card);if(!n)return;let r=e.getAttribute(`statusfield`)??``,i=t.getAttribute(`statusvalue`)??``;r===``||i.trim()===``||K(n.row,r).trim().toLowerCase()!==i.trim().toLowerCase()&&(dt(e,r,n.pindex,i),Je(n.row,r,i),ct(e))}function pt(e){lt.has(e)||(lt.add(e),e.addEventListener(`dragstart`,t=>{let n=t.composedPath().find(e=>e instanceof HTMLElement&&X.has(e))??null;n&&(Z={card:n,board:e},t.dataTransfer?.setData(`text/plain`,X.get(n)?.pindex??``),t.dataTransfer&&(t.dataTransfer.effectAllowed=`move`))}),e.addEventListener(`dragend`,()=>{Z=null}),e.addEventListener(`dragover`,t=>{Z?.board===e&&ut(e,t)&&(t.preventDefault(),t.dataTransfer&&(t.dataTransfer.dropEffect=`move`))}),e.addEventListener(`drop`,t=>{let n=ut(e,t);n&&(t.preventDefault(),ft(e,n),Z=null)}))}function mt(){Ze()&&et.forEach(ct)}function ht(){if(nt)return;nt=!0,Qe();let e=Y();e.Erstellen=()=>{$e(),mt()},e.initData=e.Erstellen,e.ReloadData=()=>mt();let t=0,n=setInterval(()=>{t+=1,Ze()?(clearInterval(n),$e(),mt()):t>100&&clearInterval(n)},300)}function gt(e){e.hasAttribute(`data-ff-editor`)||(et.add(e),pt(e),ht(),Ze()&&ct(e))}function _t(e){et.delete(e)}var Q=W.blockType,$=V.blockType,vt=class extends R{static{this.blockType=`kanban`}static{this.tagName=`ff-kanban`}static{this.displayName=`Kanban`}static{this.category=`anzeige`}static{this.acceptsChildren=!0}static{this.allowedChildTypes=[Q]}static{this.childDirection=`row`}static{this.containerHint=!1}static{this.addChildButton={label:`Spalte`,childType:Q}}static{this.acceptsDataSource=!0}static{this.defaultProps={width:`fill`,source:``,statusField:``}}static{this.customProperties=[{attributeName:`statusField`,name:`Spalten aus Feld`,description:`Feld der Datenquelle, dessen Wert bestimmt, in welcher Spalte eine Zeile landet.`,isArray:!1,maxLength:0,kind:`field`}]}static{this.defaultChildren=[{type:Q,props:{heading:`Offen`,variant:`warning`},children:[{type:$,props:{heading:`Rückruf Fr. Wagner`,text:`Befund Minka besprechen`,chipVariant:`warning`,chipText:`Wartet seit 2 Tagen`}},{type:$,props:{heading:`Rechnung Nr. 5012 prüfen`,text:`Position Narkose fehlt`,chipVariant:`danger`,chipText:`Überfällig`}},{type:$,props:{heading:`Impfpass nachtragen`,text:`Buddy · Golden Retriever`,chipVariant:`info`,chipText:`Heute`}}]},{type:Q,props:{heading:`In Arbeit`,variant:`info`},children:[{type:$,props:{heading:`Röntgenbilder anfordern`,text:`Klinik Dr. Steiner, Fall Rocky`,chipVariant:`info`,chipText:`Angefragt`}}]},{type:Q,props:{heading:`Fertig`,variant:`success`},children:[{type:$,props:{heading:`Laborprobe versendet`,text:`Nala · Blutbild groß`,chipVariant:`success`,chipText:`Erledigt`}},{type:$,props:{heading:`Bestellung Verbandsmaterial`,text:`Lieferung bestätigt für Montag`,chipVariant:`success`,chipText:`Erledigt`}}]}]}static{this.styles=[R.styles,o`
      ...
      slot { display: contents; }
    `]}render(){return E`<div class="board"><slot></slot></div>`}connectedCallback(){super.connectedCallback(),gt(this)}disconnectedCallback(){super.disconnectedCallback(),_t(this)}};R.defineAndRegister(vt);var yt=class extends R{constructor(...e){super(...e),this.text=`Neuer Text`}static{this.blockType=`text`}static{this.tagName=`ff-text`}static{this.displayName=`Textblock`}static{this.category=`anzeige`}static{this.defaultProps={text:`Neuer Text`}}static{this.customProperties=[]}static{this.styles=[R.styles,o`
      span {
        display: block;
        min-width: 1ch;
        color: var(--se-ink);
    ...
    `]}render(){return E`<span
      data-ff-editable
      @dblclick=${e=>this.inlineEdit(e,`text`)}
    >${this.text}</span>`}};L([I()],yt.prototype,`text`,void 0),R.defineAndRegister(yt)})();
...
`,z=class extends L{constructor(...e){super(...e),this.chipVariant=`info`,this.heading=`Rückruf Fr. Wagner`,this.time=`09:15`,this.meta=`Katze · EKH`,this.text=`Befund Minka besprechen`,this.chipText=`Heute`,this.headingField=``,this.timeField=``,this.metaField=``,this.textField=``,this.chipTextField=``}static{this.blockType=`card`}static{this.tagName=`ff-card`}static{this.displayName=`Karte`}static{this.category=`anzeige`}static{this.allowedParentTypes=[`kanban-spalte`]}static{this.showInPalette=!1}static{this.defaultProps={chipVariant:`info`,heading:`Rückruf Fr. Wagner`,time:`09:15`,meta:`Katze · EKH`,text:`Befund Minka besprechen`,chipText:`Heute`,headingField:``,timeField:``,metaField:``,textField:``,chipTextField:``}}static{this.bindableSpots=[{prop:`heading`,label:`Titel`},{prop:`time`,label:`Zeit`},{prop:`meta`,label:`Meta-Zeile`},{prop:`text`,label:`Textzeile`},{prop:`chipText`,label:`Chip`}]}static{this.customProperties=[tt(`chipVariant`,`Bedeutung des Chips auf der Karte — bestimmt die Chip-Farbe.`)]}static{this.styles=[L.styles,nt,o`
    ...
    >${this.placeholder}</span>`}controlTpl(e){switch(e){case`textarea`:return T`<textarea class="ctrl" @input=${this.onInput}></textarea>`;case`select`:{let e=this.options.split(`,`).map(e=>e.trim()).filter(e=>e!==``);return T`<select class="ctrl">
          ${e.length===0?T`<option>(keine Optionen)</option>`:e.map(e=>T`<option>${e}</option>`)}
    ...
    </div>`}};I([F()],V.prototype,`variant`,void 0),I([F()],V.prototype,`heading`,void 0),I([Ne()],V.prototype,`_count`,void 0),L.defineAndRegister(V);function H(e){return typeof e==`object`&&!!e}function st(e,t){if(!(!Array.isArray(e)||t===``)){for(let n of e)if(!(!H(n)||n.id!==t)&&!(typeof n.name!=`string`||typeof n.tableId!=`string`))return{id:t,name:n.name,tableId:n.tableId,indexField:typeof n.indexField==`string`?n.indexField:``}}}function ct(e,t){if(!(!Array.isArray(e)||t===``)){for(let n of e)if(!(!H(n)||n.id!==t)&&!(typeof n.verb!=`string`||!Ve.includes(n.verb))&&!(typeof n.nr!=`string`||n.nr===``)&&!(!Array.isArray(n.params)||n.params.some(e=>typeof e!=`string`)))return{id:t,verb:n.verb,nr:n.nr,params:n.params}}}function U(e){return e==null?``:String(e).trim()}function W(e,t){if(!H(e)||t===``)return``;let n=t.trim(),r=U(e[n]);if(r!==``)return r;for(let t of Object.keys(e))if(t===n||t.startsWith(`${n}_`)||t.endsWith(`_${n}`)){let n=U(e[t]);if(n!==``)return n}let i=/^(\d+)_(\d+)$/.exec(n);if(!i)return``;let a=U(e.SATZNEU??e.SATZ??e.satzneu??e.satz??e.RAW??e.raw);if(a===``)return``;let o=Number(i[1]),s=Number(i[2]);return s<=0?``:a.substring(o,o+s).trim()}function lt(e,t,n){if(!H(e)||t===``)return!1;let r=t.trim(),i=!1;for(let t of Object.keys(e))(t===r||t.startsWith(`${r}_`)||t.endsWith(`_${r}`))&&(e[t]=n,i=!0);let a=/^(\d+)_(\d+)$/.exec(r);if(a){let t=[`SATZNEU`,`SATZ`,`satzneu`,`satz`,`RAW`,`raw`].find(t=>typeof e[t]==`string`);if(t){let r=e[t],o=Number(a[1]),s=Number(a[2]);if(s>0){let a=n.length>s?n.slice(0,s):n.padEnd(s,` `),c=r.length<o?r.padEnd(o,` `):r;e[t]=c.slice(0,o)+a+c.slice(o+s),i=!0}}}return i}function G(e){if(!H(e))return Array.isArray(e)?e:[];let t=[e.Zeilen,e.zeilen,e.Saetze,e.saetze,e.Rows,e.rows,e.Daten,e.daten];for(let e of t){if(Array.isArray(e))return e;if(typeof e==`string`)try{let t=JSON.parse(e);if(Array.isArray(t))return t}catch{}}return[]}function K(e,t){return U(e).toLowerCase()===t.trim().toLowerCase()}function ut(e,t,n){if(!H(e)||!H(e.Daten))return[];let r=e.Daten,i=r.SEFileLoop;if(Array.isArray(i)){for(let e of i)if(H(e)&&(K(e.ALIAS,t)||K(e.alias,t))){let t=G(e);if(t.length>0)return t}}else if(H(i))for(let e of Object.keys(i)){let n=i[e];if(K(e,t)||H(n)&&(K(n.ALIAS,t)||K(n.alias,t))){let e=G(n);if(e.length>0)return e}}let a=r.Tabellen;if(H(a)){let e=[t,t.toUpperCase(),t.toLowerCase(),n];for(let t of e)if(t!==``&&t in a){let e=G(a[t]);if(e.length>0)return e}for(let e of Object.keys(a))if(K(e,t)){let t=G(a[e]);if(t.length>0)return t}}return[]}function dt(e){let t=e;if(typeof t==`string`)try{t=JSON.parse(t)}catch{return}if(!H(t)||!H(t.Daten))return;let n=t.Daten;if(!(!n.SEFileLoop&&!n.Tabellen&&!n.ErpApiCall))return n}function ft(e){let t=e;if(typeof t==`string`)try{t=JSON.parse(t)}catch{return}if(!(!H(t)||!H(t.MSG)))return t.MSG.DATA}function pt(e,t){let n=e.trim().toLowerCase();if(n!==``)for(let e=0;e<t.length;e++){let r=t[e].trim().toLowerCase();if(r!==``&&r===n)return e}return-1}function mt(e){return e.findIndex(e=>(e??``).trim()===`ja`)}function q(){return globalThis}function J(){let e=q();return H(e.SEDATA)&&H(e.SEDATA.Daten)}function ht(){let e=q();try{e.selib?.Json?.InitializeERPConnection?.()}catch{}try{typeof e.InitialisiereSchnittstelle==`function`&&e.InitialisiereSchnittstelle()}catch{}}function gt(){let e=q();try{typeof e.ResetDataBasis==`function`&&e.ResetDataBasis()}catch{}try{typeof e.InitialisiereDatenBasis==`function`&&e.InitialisiereDatenBasis()}catch{}}var _t=new Set,vt=new WeakMap,yt=!1,bt=V.tagName,xt=z.tagName,Y=`data-ff-nicht-zugeordnet`,St=`Nicht zugeordnet`;function Ct(e){return Array.from(e.children).filter(e=>e.tagName.toLowerCase()===bt&&!e.hasAttribute(Y))}function wt(e){return Array.from(e.children).filter(e=>e.tagName.toLowerCase()===xt)}function Tt(e){return Ie().find(t=>t.tagName===e.toLowerCase())?.bindableSpots??[]}function Et(e){let t=e.getAttribute(`source`)??``,n=e.getAttribute(`statusfield`)??``;if(t===``||n===``)return;let r=st(q().FF_DATA_SOURCES,t);if(!r)return;let i=Ct(e);if(i.length===0)return;let a=vt.get(e);if(!a){let t=e.querySelector(`template[data-ff-template]`)?.content.firstElementChild??e.querySelector(xt);t&&(a=t.cloneNode(!0),vt.set(e,a))}if(!a)return;let o=ut(q().SEDATA,r.name,r.tableId),s=i.map(e=>e.getAttribute(`heading`)??``),c=Tt(a.tagName),l=mt(i.map(e=>e.getAttribute(`auffang`)));e.querySelectorAll(`[`+Y+`]`).forEach(e=>e.remove());let u=null,d=()=>(u||(u=document.createElement(bt),u.setAttribute(`heading`,St),u.setAttribute(Y,``),u.setAttribute(`style`,i[0].getAttribute(`style`)??`flex-grow:1;flex-basis:0;min-width:0`),e.appendChild(u)),u);for(let e of i)wt(e).forEach(e=>e.remove());for(let e of o){let t=a.cloneNode(!0),o=pt(W(e,n),s);(o>=0?i[o]:l>=0?i[l]:d()).appendChild(t);for(let n of c){let r=t.getAttribute(`${n.prop.toLowerCase()}field`)??``;r!==``&&(t[n.prop]=W(e,r))}let u=W(e,r.indexField);u!==``&&(X.set(t,{row:e,pindex:u}),t.draggable=!0)}}var X=new WeakMap,Z=null,Dt=new WeakSet;function Ot(e,t){for(let n of t.composedPath())if(n instanceof HTMLElement&&n.tagName.toLowerCase()===bt&&e.contains(n))return n;return null}function kt(e){let t=q(),n=ct(t.FF_RELATIONS,e.getAttribute(`putrelation`)??``),r=st(t.FF_DATA_SOURCES,e.getAttribute(`source`)??``);if(!(!n||!r))return{template:n,relId:He(r.tableId)}}function At(e,t,n,r){let i=q();if(typeof i.basisHTML_SND_MSG!=`function`)return;let a=We(t);a&&i.basisHTML_SND_MSG(e.template.verb,{NR:e.template.nr,PARAMS:Ge(e.template,{FELD_POS:a.pos,FELD_LEN:a.len,PINDEX:n,DROP_PINDEX:n,RELID:e.relId,VALUE:r,NOW_DATE:Ue(new Date)})})}function jt(e,t){if(!Z||Z.board!==e)return;let n=X.get(Z.card);if(!n)return;let r=e.getAttribute(`statusfield`)??``,i=t.getAttribute(`heading`)??``;if(r===``||i.trim()===``||W(n.row,r).trim().toLowerCase()===i.trim().toLowerCase())return;let a=kt(e);a&&(At(a,r,n.pindex,i),lt(n.row,r,i),Et(e),R(e,`onCardDrop`,{PINDEX:n.pindex,VALUE:i}))}function Mt(e){Dt.has(e)||(Dt.add(e),e.addEventListener(`click`,t=>{let n=t.composedPath().find(e=>e instanceof HTMLElement&&X.has(e))??null;n&&R(e,`onCardClick`,{PINDEX:X.get(n)?.pindex??``})}),e.addEventListener(`dragstart`,t=>{let n=t.composedPath().find(e=>e instanceof HTMLElement&&X.has(e))??null;n&&(Z={card:n,board:e},t.dataTransfer?.setData(`text/plain`,X.get(n)?.pindex??``),t.dataTransfer&&(t.dataTransfer.effectAllowed=`move`))}),e.addEventListener(`dragend`,()=>{Z=null}),e.addEventListener(`dragover`,t=>{let n=Ot(e,t);Z?.board===e&&n&&!n.hasAttribute(Y)&&(t.preventDefault(),t.dataTransfer&&(t.dataTransfer.dropEffect=`move`))}),e.addEventListener(`drop`,t=>{let n=Ot(e,t);!n||n.hasAttribute(Y)||(t.preventDefault(),jt(e,n),Z=null)}))}function Q(){J()&&_t.forEach(Et)}var Nt=!1;function Pt(e){if(!Nt){Nt=!0;try{let t=typeof e==`string`?e:JSON.stringify(e),n=document.createElement(`textarea`);n.id=`ff-se-diagnose`,n.readOnly=!0,n.value=t??``,n.style.cssText=`display:none;position:fixed;left:8px;right:8px;bottom:8px;height:40vh;z-index:99999;font:11px monospace;`,document.body.appendChild(n)}catch{}}}function Ft(e){let t=dt(e);if(!t)return;let n=q();H(n.SEDATA)||(n.SEDATA={}),n.SEDATA.Daten=t,Pt(e),gt(),Q()}function It(e=0){let t=q();if(typeof t.basisHTML_REGISTER==`function`){try{t.basisHTML_SetConsoleLog?.(!0,!0)}catch{}try{t.basisHTML_REGISTER(e=>{Ft(e)},document.title,`1.0`)}catch{}return}e<400&&setTimeout(()=>{It(e+1)},25)}function Lt(){if(yt)return;yt=!0,ht();let e=q();e.Erstellen=()=>{gt(),Q()},e.initData=e.Erstellen,e.ReloadData=()=>Q(),It(),window.addEventListener(`message`,e=>{if(typeof q().basisHTML_REGISTER==`function`)return;let t=ft(e.data);t!==void 0&&Ft(t)},!0),document.addEventListener(`keydown`,e=>{if(e.ctrlKey&&e.altKey&&e.key.toLowerCase()===`d`){let e=document.getElementById(`ff-se-diagnose`);e&&(e.style.display=e.style.display===`none`?`block`:`none`)}});let t=0,n=setInterval(()=>{t+=1,J()?(clearInterval(n),gt(),Q()):t>100&&clearInterval(n)},300)}function Rt(e){e.hasAttribute(`data-ff-editor`)||(_t.add(e),Mt(e),Lt(),J()&&Et(e))}function zt(e){_t.delete(e)}var $=V.blockType,Bt=class extends L{static{this.blockType=`kanban`}static{this.tagName=`ff-kanban`}static{this.displayName=`Kanban`}static{this.category=`anzeige`}static{this.acceptsChildren=!0}static{this.allowedChildTypes=[$]}static{this.childDirection=`row`}static{this.containerHint=!1}static{this.addChildButton={label:`Spalte`,childType:$}}static{this.templateChild={type:z.blockType,label:`Muster`}}static{this.resizableHeight=!0}static{this.acceptsDataSource=!0}static{this.blockEvents=[{key:`onCardClick`,name:`Karte angeklickt`},{key:`onCardDrop`,name:`Karte verschoben`}]}static{this.defaultProps={width:`fill`,height:`auto`,source:``,statusField:``,putRelation:`standard-put`}}static{this.bindingRoute={fieldProp:`statusField`}}static{this.customProperties=[{attributeName:`statusField`,name:`Einsortieren nach`,description:`Das Feld der Datenquelle, dessen Inhalt bestimmt, in welche Spalte ein Eintrag kommt (z. B. Behandlungszimmer).`,isArray:!1,maxLength:0,kind:`field`,hiddenInInspector:!0},{attributeName:`putRelation`,name:`Beim Verschieben zurückschreiben über`,description:`Relation-Vorlage, über die eine verschobene Karte den Titel ihrer neuen Spalte ins Sortier-Feld zurückschreibt.`,isArray:!1,maxLength:0,kind:`relation`,requiresDataSource:!0,hiddenInInspector:!0}]}static{this.defaultChildren=[{type:$,props:{heading:`Offen`,variant:`warning`},children:[{type:z.blockType}]},{type:$,props:{heading:`In Arbeit`,variant:`info`}},{type:$,props:{heading:`Fertig`,variant:`success`}}]}static{this.styles=[L.styles,o`

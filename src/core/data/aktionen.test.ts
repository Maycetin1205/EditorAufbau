...
describe('Neuen Satz anlegen (CREATE_RECORD, 2026-07-20)', () => {
  const getRel: RelationTemplate = {
    id: 'rel-get', name: 'Neuer Satz', verb: 'GET_RELATION', nr: '0640',
    params: ['ID0001'], allowExtraParams: false,
  }
  const step = {
    id: 'c1',
  ...
  it('createStep legt einen leeren Schritt an', () => {
    expect(createStep('CREATE_RECORD')).toMatchObject({
      type: 'CREATE_RECORD', dataSourceId: '', getRelationId: '', relationId: '',
    })
  })

  it('transportiert Quelle + beide Vorlagen — ohne Editor-id', () => {
    const raw = serializeBlockEvents({ onClick: [step] }, ['onClick'])
    expect(raw).not.toContain('"id"')
    expect(parseBlockEvents(raw)).toEqual({
      onClick: [{
        type: 'CREATE_RECORD',
    ...
    // Persistenz-Weg: gespeicherter Schritt bleibt vollständig erhalten.
    expect(sanitizeBlockEvents({ onClick: [step] }, ['onClick']))
      .toEqual({ onClick: [step] })
    // Fehlende Vorlagen-id wird verworfen (nie raten).
    expect(sanitizeBlockEvents({
      onClick: [{ ...step, getRelationId: undefined }],
    }, ['onClick'])).toBeUndefined()
  })

  it('stepProblem: Quelle, Hol- und Schreib-Vorlage müssen stehen und passen', () => {
    expect(stepProblem({ ...step, dataSourceId: '' })).toContain('keine Datenquelle')
    expect(stepProblem(step, undefined, [])).toContain('geloeschte Datenquelle')
    expect(stepProblem({ ...step, getRelationId: '' })).toContain('keine Hol-Vorlage')
    expect(stepProblem({ ...step, relationId: '' })).toContain('keine Schreib-Vorlage')
    // Hol-Vorlage MUSS ein GET sein, Schreib-Vorlage MUSS ein Schreib-Verb sein.
    expect(stepProblem(step, [{ ...relation, id: 'rel-get' }, relation]))
      .toContain('als Hol-Vorlage eine GET-Vorlage')
    expect(stepProblem(step, [getRel, { ...getRel, id: 'rel-1' }]))
      .toContain('Schreib-Vorlage (PUT/PUTADD)')
    // Alles stimmig -> null.
    expect(stepProblem(step, [getRel, relation], [quelle])).toBeNull()
  })
})

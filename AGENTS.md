...
- ⚑ **STABILISIERUNG — Architektur- & Bedienkorrektur (vor Kap. 6)**
  ...
  **⚑ EINSCHUB (2026-07-10):** Das K-Programm (unten) hat VORRANG — Kanban
  wird KOMPLETT fertig gebaut, bevor irgendetwas anderes beginnt. S3-Rest
  ist im K-Programm aufgegangen (K4 + K6), der Kanban-Teil von S1 in K7;
  der Stabilisierungs-Rest (S1b–e allgemein, S2, S4, S5, S6) ist hinter K8
  eingereiht.
  ...
  - **S3 Kanban-Modell + Layout korrigieren** `[kritisch]`: Der zentrale
    ...
    **UMBRECHEN in die nächste Zeile statt intern zu scrollen** (KEIN
    ...
    - ✅ **S3-Musterkarte (2026-07-09, Nutzer-Entscheidungen per Fragebogen:
      ...
      **→ S3-Rest AUFGEGANGEN im K-Programm (2026-07-10):** Spalten-VORSCHAU
      im Editor = K4, geschlossene Bindungs-UI (Punkte 1–6) = K6. Die
      Layout-Entscheidung „Umbruch statt Scroll" vom 2026-07-09 ist ERSETZT
      durch K0/Entscheidung A: alle Spalten IMMER nebeneinander sichtbar,
      keine Mindestbreite, kein Umbruch, kein horizontaler Scroll.
  ...
  - **Leitplanken:** Tests werden NIE gelöscht/abgeschwächt — die grünen
    ...
    S1–S6 es brauchen). **→ Reihenfolge seit 2026-07-10: erst K-Programm
    (unten), dann Stabilisierungs-Rest, dann Kap. 6.**
- ⌖ **AKTUELLES PROGRAMM (beschlossen 2026-07-10): Kanban KOMPLETT
  (K0–K8).** Kanban wird KOMPLETT fertig gebaut, bevor irgendetwas anderes
  beginnt (auch vor dem Stabilisierungs-Rest). Pakete in dieser Reihenfolge,
  jedes einzeln: Plan → „go" → bauen → Tests → committen. Ergebnis-Beleg =
  Screenshots Editor vs. exportierte Maske bei SE-typischer Fensterbreite
  (~800px). **Verhaltensreferenz fürs Board (lokal):**
  `C:\Users\mu.aycetin\Desktop\behandlung-umbau\empfang\index.basis.source.html`
  — NUR das Board; Karteikarte, Navi, Terminkalender ausdrücklich NICHT
  übernehmen. Achtung: die Datei hat zwei `.vboard`-Definitionen; die
  ...
  **⚑ KURSKORREKTUR (2026-07-10, Nutzer — ERSETZT die K-Reihenfolge unten):**
  ...
  - **K0 Geometrie (Korrektur 2026-07-10: Codex hat NIE
    ...
    Spalte (Spaltenrumpf `flex:1` + `min-height:0` + `overflow-y:auto`).
    Höhe als NEUES Registry-Konzept analog zur Breite (`resizableHeight`,
    nur Kanban schaltet ein): Inspector „Höhe: Automatisch / Fest (px)" +
  ...
  - **K4 Vorschau-Karten** `[kritisch]`: Spalten zeigen im Editor generierte
    Vorschau-Karten aus den Beispielzeilen — über DENSELBEN puren
    Hydrations-Helfer wie die SE-Laufzeit (aus seRuntime herauslösen; der
    Editor täuscht NIE SEDATA-Globals vor). Klar als Vorschau markiert, nie
    im Export. (Erledigt den S3-Rest „Spalten-Vorschau".)
  - **K5 Musterkarte erweitern** `[kritisch]`: ~5 Stellen nach
    Empfang-Vorbild (Zeit/Titel/Meta/Zusatz/Chip). Zielbild VORHER in
    `dashboard/stilprobe.html` + Nutzer-Abnahme. Live-Verknüpfungen über
    andere Quellen (Besitzer/Tier) sind Kap. 7 — NICHT bauen.
  - **K5b Spalten-Unterbereiche** `[kritisch]`: je Spalte Schalter
    „Unterbereiche an/aus"; an = Spalte trägt eine LISTE von Datenwerten,
    jeder Wert = sichtbarer Unterbereich mit eigenem Anzeigenamen + eigener
    Drop-Zone (Drop schreibt exakt dessen Wert). Anzahl per Plus/Kreuzchen.
    Generischer Name „Unterbereiche" — NIE „Zimmer" im Baustein (Zimmer ist
    nur die Verwendung des Nutzers; Empfang-Spalte „Behandlung" = 3 Werte
    Behandlungszimmer 1–3).
  - **K6 Geführte Bindungsstrecke + Fertig-Anzeige** `[kritisch]`: die
    verstreuten Bindungsorte (Quelle → Spaltenfeld → Spaltenwerte/
    Unterbereiche → Kartenstellen → Schreibrelation → Vorschau) als EINE
    geführte, abhakbare Strecke am Board. WICHTIG: In K6 wird das
    EINBINDUNGS-KONZEPT festgelegt (wie Datenquellen, Feldzuweisungen,
    Relationen und künftige AKTIONEN (Kap. 8) am Block angeboten werden —
    Aktionen nur als Erweiterungspunkt mitdenken, NICHT bauen). Konzept dem
    Nutzer als Bediener-Durchspiel vorlegen, Abnahme abwarten. (Erledigt den
    S3-Rest „geschlossene Bindungs-UI Punkte 1–6".)
  - **K7 Export-Preflight — ERST NACH K6-ABNAHME** (Nutzer-Regel:
    Export-Arbeiten erst, wenn festgelegt ist, wie alles eingebunden wird):
    Export blockiert verständlich bei unvollständiger Bindung (Spaltenfeld
    fehlt, Spalte/Unterbereich ohne Wert, unbekannte Relation); HTML +
    SEvariablen-JSON als EIN Paket geprüft (ehemals „K7.5" — kein eigenes
    Paket). Deckt den Kanban-Teil von S1 ab; der allgemeine S1-Rest
    (doppelte Aliasse, quellenart-spezifische Pflichtfelder, referenzielle
    Integrität allgemein) bleibt hinter K8.
  ...
  **Bewusste Grenzen (K-Programm):** keine Karteikarte/DetailCard, keine
  ...
  erzwingt. Bibliotheks-Review (Button/Text/Chip/Bereich/Infobox) bleibt
  geparkt bis nach K8.
- **Kap. 6 — weitere Blöcke** `[Muster kritisch, Ausbau mechanisch]`
  (GESPERRT bis K-Programm (K8) + Stabilisierungs-Rest fertig):

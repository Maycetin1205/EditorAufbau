...
You are now in **implementation mode** for **Aufbau-Editor (EditorAufbau)**.

> Projekt-Kontext: Regeln und Stand leben in `CLAUDE.md` (gewinnt bei
> Widerspruch). Rückmeldungen an den Nutzer auf Deutsch und fachlich.
...
```bash
git fetch origin   # Regel 8: Pflicht VOR Arbeitsbeginn — ist origin voraus, erst dessen Stand ansehen und zusammenführen
...
```bash
npx eslint src 2>&1 | tee /tmp/_trip2-lint.txt
npx tsc -b 2>&1 | tee /tmp/_trip2-typecheck.txt
```
...
Das volle Prüfbündel läuft EINMAL hier — nie zwischendurch (Regel 9; die
Batch-Mikro-Gates oben sind bewusst nur lint + typecheck). Die Suite ist
bewusst klein — nicht ohne Absprache aufblähen.

### 3. Integration impact check

Export oder Masken-Laufzeit berührt (`src/export/`, `serializer`, `ff-runtime`,
`seAktionen`, `src/softengine/`, Baustein-Rendering)? Dann gilt:

- `npm run build:runtime`, falls die Masken-Laufzeit geändert wurde (sonst schlägt der Veralten-Wächter an)
- Referenzabzug muss grün sein; ABSICHTLICHE Export-Änderung → Referenz mit `npx vitest run -u` erneuern (der Datei-Diff macht die Maskenänderung im Commit sichtbar)
- Test-Bremse: dann EIN schlanker e2e-Kreislauf-Test — keine Einzeltest-Flut; reine Editor-Bedienpakete bekommen KEINE neuen e2e
- **SE-Echttest durch den Nutzer anmelden** (wird auf Nutzer-Wunsch gebündelt, nie einzeln nachfassen)
